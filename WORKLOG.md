...
- 每日回声改为服务端持久任务 + SSE 预览 + polling 恢复；刷新、离页和重新进入不取消任务，成功后原子创建站内通知。
- 数据一致性使用 source hash、数据库活动任务/用户运行唯一键和租约锁；任务终态清理临时正文，服务端不直接覆盖 local-first 日记。
- 本期 Android 只复用现有本地通知通道，不接 EMAS/FCM。生产功能开关默认关闭，部署与真机验收待办见 `vault/TODO.md`，完整记录见 `vault/notes/daily/2026-07-11.md`。
...
- 日记导出中英文重叠和背景图一致性修复已经正式推到 Android `1.0.21 / versionCode 23`。
- 线上更新清单与本地说明已同步，后续再改导出链路时要继续保留 APK 实测，不要只看浏览器预览。
