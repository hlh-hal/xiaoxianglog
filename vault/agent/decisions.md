...
- 账号主键始终是小象日志 `User.id`；邮箱密码与微信不是两个可合并账号，而是同一个用户的不同登录凭据。微信身份单独存入 `ExternalIdentity`，日记及社交数据不迁移、不复制。
- 微信不提供可信邮箱映射。已有邮箱账号只能在邮箱登录后，经当前邮箱验证码二次验证绑定；微信首次登录只允许用未注册邮箱创建新账号，禁止未登录状态自动认领或合并旧账号。
- 微信授权 code 必须由服务端使用 AppSecret 交换；AppSecret、access token、OpenID/UnionID 不进入前端日志。未绑定授权使用服务端一次性、短时、哈希落库的 `ExternalAuthGrant` 过渡。
- Android 第一版使用官方 OpenSDK 和自研 Capacitor 桥接；普通 Web/PWA 不展示入口。生产默认关闭 `WECHAT_LOGIN_ENABLED`，回滚优先关开关并保留身份表，避免已绑定用户丢失映射。
...
- 前端发布采用“不可变资源先行、入口最后激活”的两阶段模型。`index.html` 只能引用已经通过 FTP SIZE、公网 SHA256 和 JS 语法校验的哈希资源；任一门禁失败都不得切换入口。
- `index.html` 与 `sw.js` 禁止直接覆盖上传。当前 FTP 不支持 RNTO 覆盖已有文件，统一使用“上传随机临时文件 → 旧文件改名为 rollback 备份 → 临时文件改名就位 → 最终校验 → 删除备份”，失败时恢复备份。
- “FTP 返回成功”“资源 HTTP 200”都不是发布完成证据。前端发布必须以公网反向下载哈希一致和全新浏览器 React 挂载、无 page error 为最终完成条件。
