...
这份文件是 Codex/Agent 的唯一项目入口，用于新会话快速了解本项目的技术栈、代码约定、持久记忆规则和用户偏好。开始改代码前请先读它，再观察相关文件的现有写法。

## 持久工作记忆

把 `vault/` 当作本项目的共享工作记忆和跨会话交接源。它用于保存重要但不适合只留在聊天记录里的上下文，例如待办、关键决策、阻塞项、负责人、日期、常用链接、项目背景和下次接手提示。

使用原则：

- 优先更新已有笔记，不要为同一主题到处创建新文件。
- 没有有意义变化时，不要修改 `vault/`。
- 代码、配置和测试仍放在代码仓库对应目录；滚动上下文、决策和交接信息放进 `vault/`。
- 不要把密钥、真实 token、私密账号密码、数据库文件或用户隐私数据写入 `vault/`。
- 记录时尽量写清日期、来源、当前状态、下一步和相关文件链接。

强制写入规则：

- 只要本次任务修改了代码、部署配置、测试、数据结构、同步逻辑、认证、通知、PWA、Android、线上配置或用户可见流程，就必须在结束前检查是否需要更新 `vault/`。
- 如果改动超过 3 个文件、产生了新的验证命令、修复了高影响 bug、发现了坑、改变了交接方式或留下待办/风险，必须更新 `vault/`。
- `WORKLOG.md` 只作为旧交接日志和近期索引；有长期价值的内容必须优先写入 `vault/`。不要只写 `WORKLOG.md` 而不写 `vault/`。
- 如果判断“不需要写 vault”，最终回复里说明原因，例如“本次只是一次性查询，无长期上下文”。

目录约定：

- `vault/TODO.md`：跨会话待办、阻塞项和下次优先事项。
- `vault/projects/`：项目级背景、里程碑、重要决策和交接说明。
- `vault/people/`：人物、负责人、协作偏好和联系上下文；只记录工作必要信息。
- `vault/agent/`：给后续 Agent 的工作规则、决策记录和踩坑记录。
- `vault/notes/`：临时笔记、每日摘要、草稿和复盘。

更新建议：

- 开始复杂任务前，查看 `vault/TODO.md`、`vault/agent/memory-workflow.md` 和相关 `vault/projects/*.md`。
- 任务结束前，按 `vault/agent/memory-workflow.md` 的清单检查并同步对应笔记。
- 如果只是小修、一次性查询或无新增长期价值，不写知识库。
...
1. 新窗口先看 `AGENTS.md`、`vault/TODO.md`、`vault/agent/memory-workflow.md` 和相关 `vault/projects/*.md`，了解项目规则、最近重大变更、阻塞项和待办风险。
2. 再看 `WORKLOG.md`、`package.json`、相关页面或路由、相关 service，再动手。
3. 如果遇到乱码，先判断是否和任务有关；无关就绕开。
4. 如果是 UI 任务，改完启动 dev server 做视觉验证。
5. 如果是数据或接口任务，同时检查前端 service、后端 route、Prisma schema。
6. 完成后说明改了哪些文件、跑了哪些验证、还有哪些风险；如果发生重大变更或踩到关键坑，必须同步更新 `vault/` 中对应笔记，必要时也更新 `WORKLOG.md`。
